...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErr(data.detail || "Login failed. Please check your credentials.");
        return;
      }
      // Save token & user info (remember = persist across browser restarts)
      saveAuth(data.token, data.user, rem);
      ...
      <div className="ll-page" style={{ background: t.bg }}>
        ...
        <div className="ll-wave">
          <svg viewBox="0 0 300 120" fill="none">
            <defs>
              <linearGradient id="g1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#39d98a" stopOpacity=".9" /><stop offset="100%" stopColor="#39d98a" stopOpacity="0" /></linearGradient>
              <linearGradient id="g2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#a78bfa" stopOpacity=".8" /><stop offset="100%" stopColor="#a78bfa" stopOpacity="0" /></linearGradient>
            </defs>
            {([
              ["M60,110 Q70,20 80,110 Q90,20 100,110", "url(#g1)", "0s"],
              ["M90,110 Q105,10 120,110 Q135,10 150,110", "url(#g2)", ".3s"],
              ["M130,110 Q145,30 160,110 Q175,30 190,110", "url(#g1)", ".6s"],
              ["M170,110 Q182,50 194,110", "url(#g2)", ".9s"],
        ...
        <div className="ll-card"
          ...
          <h1 className="ll-title" style={{ color: t.text }}>Sign In to Your Account</h1>
          <p className="ll-sub" style={{ color: t.muted }}>Your AI-powered lecture assistant awaits</p>
          ...
          <form onSubmit={submit} className="ll-form">
            ...
            <div className="ll-field">
              <input type={show ? "text" : "password"} placeholder="Password" value={pass} onChange={e => setPass(e.target.value)}
                className="ll-input" autoComplete="current-password"
                style={{ background: dark ? "rgba(255,255,255,.05)" : "rgba(0,0,0,.04)", border: `1px solid ${t.border}`, color: t.text }} />
              <button type="button" className="ll-ico ll-eye" onClick={() => setShow(s => !s)}>
                {show ? "🙈" : "🔒"}
              </button>
            </div>

            <div className="ll-row">
              <label className="ll-rem-label" style={{ color: t.muted }} onClick={() => setRem(r => !r)}>
                <div className="ll-chk"
                  style={{ border: `1.5px solid ${rem ? t.accent : t.border}`, background: rem ? t.accentBg : "transparent" }}>
                  {rem && <span style={{ color: t.accent, fontSize: ".7rem" }}>✓</span>}
                </div>
                Remember Me
              </label>
              <a href="/forgot-password" className="ll-forgot" style={{ color: t.muted }}
            ...
            <button type="submit" className="ll-continue" disabled={busy === "email"}
              ...
              {busy === "email" ? <span className="ll-spin" /> : "Continue"}
            </button>
          </form>
          ...
          <div className="ll-divider">
            ...
            <span style={{ color: t.muted, fontSize: ".82rem", fontFamily: "monospace", whiteSpace: "nowrap" }}>Or continue with</span>
            <div style={{ flex: 1, height: 1, background: t.border }} />
          </div>

          {/* OAuth row */}
          <div className="ll-oauth">
            {/* Google */}
            <button className="ll-obtn" onClick={() => oauth("google")} disabled={!!busy}
              style={{ background: dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.04)", border: `1px solid ${t.border}` }} title="Sign in with Google">
            ...
            <button className="ll-obtn" onClick={() => oauth("apple")} disabled={!!busy}
              style={{ background: dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.04)", border: `1px solid ${t.border}`, color: t.text }} title="Sign in with Apple">
            ...
            <button className="ll-obtn" onClick={() => oauth("microsoft")} disabled={!!busy}
              style={{ background: dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.04)", border: `1px solid ${t.border}` }} title="Sign in with Microsoft">
          ...
          <p className="ll-signup" style={{ color: t.muted }}>
            ...
            <a href="/signup" onClick={(e) => { e.preventDefault(); router.push("/signup"); }}
              style={{ color: t.accent, fontWeight: 700, textDecoration: "none" }}>Sign up free</a>
          </p>
        </div>
      </div>
    </>
...
function css(t: Th) {
  return `
    ...
    .ll-card{position:relative;z-index:2;border-radius:28px;padding:56px 48px 48px;width:100%;max-width:500px;
    ...
    .ll-sub{font-size:.9rem;text-align:center;margin-bottom:32px;font-family:'Space Mono',monospace;}

    .ll-err{background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);color:#fca5a5;border-radius:12px;
            padding:12px 16px;font-size:.82rem;font-family:'Space Mono',monospace;margin-bottom:18px;line-height:1.5;}
    ...
    .ll-row{display:flex;align-items:center;justify-content:space-between;}
    .ll-rem-label{display:flex;align-items:center;gap:10px;font-size:.88rem;cursor:pointer;user-select:none;font-family:'Space Mono',monospace;}
    ...
    .ll-forgot{font-size:.88rem;text-decoration:none;font-family:'Space Mono',monospace;transition:color .2s;}

    .ll-continue{width:100%;padding:17px;border:none;border-radius:14px;font-family:'Syne',sans-serif;font-size:1.08rem;font-weight:800;
                 cursor:pointer;transition:all .25s;margin-top:6px;display:flex;align-items:center;justify-content:center;}
    ...
    .ll-divider{display:flex;align-items:center;gap:14px;margin:28px 0 24px;}
    ...
    .ll-signup{text-align:center;font-size:.9rem;margin-top:24px;font-family:'Space Mono',monospace;}

    @media(max-width:540px){.ll-card{padding:40px 24px 36px;}.ll-title{font-size:1.5rem;}}
  `;
}
